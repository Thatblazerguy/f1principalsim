...
export function ServerManagementTable({
  ...
  const getCPUBars = (percentage: number, status: Server["status"]) => {
    ...
    const getBarColor = (index: number) => {
      ...
      if (status === "active") return "bg-foreground/60";
      if (status === "paused") return "bg-muted-foreground/50";
  ...
  return (
    <div className={`w-full max-w-7xl mx-auto p-6 ${className} ${isDark ? "dark" : ""}`}>
          ...
          {servers.map((server) => (
            ...
            >
              <motion.div
                ...
                whileHover={shouldReduceMotion ? undefined : { y: -1, transition: { type: "spring", stiffness: 400, damping: 25 } }}
              >
                <div
